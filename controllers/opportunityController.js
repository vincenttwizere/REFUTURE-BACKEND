...
export const getAllOpportunities = async (req, res) => {
  try {
    const { type, isRemote, isActive, search, page = 1, limit = 10 } = req.query;
...
// Inside opportunityController.js
export const checkIfSaved = async (req, res) => {
  // Example logic:
  // Check if a user has saved a given opportunity
  try {
    const { userId, opportunityId } = req.params;
    // Replace with your actual model logic
    const isSaved = await SomeModel.exists({ user: userId, opportunity: opportunityId });
